...
public static class Search
{
    ...
    {
        string searchName = "Depth First Search";

        System.Collections.Generic.Stack<(int, int)> stack = new System.Collections.Generic.Stack<(int, int)>();

        stack.Push((maze.startingPosition.row, maze.startingPosition.column));
        ...
        while (stack.Count() > 0 && !goalFound)
        {
            ...
            // Get the next tile on the maze we will move to
            (int row, int col) currentNode = stack.Pop();
            ...
            foreach ((int row, int col) neighbour in maze.GetUndiscoveredNeighbours())
            {
                // add the neighbours to front of the queue 
    ...
    public static void search(Maze maze, string searchType, bool debug)
    {
